...
const GoogleSheetsModal = (function() {
    ...
    /**
     * Fetch data from Google Sheet using GSRCDN
     * @param {string} sheetName - Name of the sheet
     * @param {string} cell - Cell reference (e.g., "B2")
     * @returns {Promise<string>} - Cell content
     */
    async function fetchCellData(sheetName, cell) {
        if (typeof GSRCDN === 'undefined') {
            throw new Error('GSRCDN library is not loaded');
        }

        try {
            // Use GSRCDN.getData which is the correct method
            const response = await GSRCDN.getData(sheetName, [cell]);
            
            console.log('GSRCDN Response:', response);

            if (response && response.length > 0 && response[0]) {
                // The response format is an array of objects
                const cellValue = response[0][cell];
                
                if (cellValue) {
                    return cellValue;
                } else {
                    throw new Error(`Cell ${cell} is empty or not found`);
                }
            } else {
     ...
     * @param {string} cell - Cell reference (e.g., "B2")
     */
    async function showCell(title, sheetName, cell) {
        if (!isInitialized) {
        ...
        try {
            const content = await fetchCellData(sheetName, cell);
...
// Auto-initialize if GSRCDN_CONFIG is available
if (typeof GSRCDN_CONFIG !== 'undefined' && GSRCDN_CONFIG.scriptUrl) {
 ...
 * GoogleSheetsModal.init('YOUR_SCRIPT_URL');
 * 
 * // Show content from a specific cell
 * GoogleSheetsModal.showCell('About Us', 'Sheet1', 'B2');
 * 
 * // Show custom content
 * GoogleSheetsModal.showContent('Welcome', '<p>Visit https://example.com for more info</p>');
 * 
 * // Close the modal
 * GoogleSheetsModal.close();
 */