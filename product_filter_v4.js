 ...
 *   },
 *   onShowMore: (displayedProducts, hasMore) => {
 *     console.log('Showing:', displayedProducts, 'Has more:', hasMore);
 *   },
 *   categoryLabels: {
 *     all: 'All Products',
...
class ProductFilterManager {
    constructor(options = {}) {
        this.config = {
            ...
            showMoreButton: options.showMoreButton || '#showMoreBtn',
            productsPerPage: options.productsPerPage || 50,
        ...
        this.currentCategory = 'all'; // Default to All Products
     ...
     */
    setupShowMoreButton() {
        const button = this._getElement(this.config.showMoreButton);
        if (!button) return;
    ...
    showMore() {
        this.currentPage++;
        const startIndex = 0;
        const endIndex = this.currentPage * this.config.productsPerPage;
        
        this.displayedProducts = this.filteredProducts.slice(startIndex, endIndex);
        const hasMore = endIndex < this.filteredProducts.length;
        
        // Update button visibility
        this.updateShowMoreButton(hasMore);
        
        // Trigger callback
        if (this.config.onShowMore) {
            this.config.onShowMore(this.displayedProducts, hasMore);
        }
        ...
        return {
            ...
            showing: this.displayedProducts.length
        };
    }
    ...
    updateShowMoreButton(hasMore) {
        ...
        if (hasMore) {
            ...
            const remaining = this.filteredProducts.length - this.displayedProducts.length;
            this._setText(button, `Show More (${remaining} remaining)`);
        } else {
    ...
    resetPagination() {
        this.currentPage = 1;
        this.displayedProducts = this.filteredProducts.slice(0, this.config.productsPerPage);
        const hasMore = this.config.productsPerPage < this.filteredProducts.length;
        this.updateShowMoreButton(hasMore);
    }
     ...
     */
    getCategories() {
        const categories = [...new Set(this.config.products
            .filter(p => p.category !== 'C1') // Skip C1 in category list
            .map(p => p.category))];
    ...
    /**
     * Get Best Sellers - evenly distributed from each category (excluding C1)
     */
    getBestSellers() {
        const limit = this.config.bestSellersLimit;
        
        // Get all products excluding C1
        const validProducts = this.config.products.filter(p => p.category !== 'C1');
    ...
    /**
     * Apply all filters (category, price, sort) - UPDATED to skip C1 and handle pagination
     */
    applyFilters() {
        ...
        // Reset pagination and get first page
        this.resetPagination();
        
        // Trigger callback with paginated products
        if (this.config.onFilterChange) {
            this.config.onFilterChange(this.displayedProducts, {
                total: this.filteredProducts.length,
                showing: this.displayedProducts.length,
                hasMore: this.displayedProducts.length < this.filteredProducts.length
            });
        }
    ...
    /**
     * Get current filtered products (all, not paginated)
     */
     ...
     */
    resetFilters() {
        this.currentCategory = 'all'; // Reset to All Products
