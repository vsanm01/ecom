/**
 * ProductRenderer - Simplified Catalog Mode
 * Version: 3.1.0 - Minimal Product Display with Quick View
 * 
 * Features:
 * - Product image, name, category, description
 * - Quick view button (bottom right)
 * - Quick view modal (image, name, category, description only)
...
(function(window) {
    ...
    const ProductRenderer = {
                ...
                button.addEventListener('click', (e) => {
                    ...
                    switch(action) {
                        case 'quickView':
                            ...
                            } else if (this.config.defaultQuickView) {
        ...
        showQuickView: function(productId) {
            ...
            const modalHTML = this.generateQuickViewModal(product);
            document.body.insertAdjacentHTML('beforeend', modalHTML);
            
            const overlay = document.querySelector('.pr-modal-overlay');
            const closeBtn = overlay.querySelector('.pr-modal-close');
            const categoryLink = overlay.querySelector('.pr-modal-category');
            ...
            if (categoryLink && this.config.onCategoryClick) {
        ...
        generateQuickViewModal: function(product) {
            const imageContent = this.generateModalImageContent(product);
            
            return `
                <div class="pr-modal-overlay">
                    <div class="pr-modal">
                        <button class="pr-modal-close" aria-label="Close">
                            <i class="fas fa-times"></i>
                        </button>
                        <div class="pr-modal-content">
                            <div class="pr-modal-image-section">
                                ${imageContent}
                            </div>
                            <div class="pr-modal-info-section">
                                ${product.category ? `
                                    <a href="#" class="pr-modal-category" data-category="${product.category}">
                                        ${product.category}
                                    </a>
                                ` : ''}
                                <h2 class="pr-modal-title">${product.title}</h2>
                                ${product.description ? `
                                    <p class="pr-modal-description">${product.description}</p>
                                ` : ''}
                            </div>
                        </div>
                    </div>
        ...
        generateModalImageContent: function(product) {
            if (product.image && this.isValidURL(product.image)) {
                return `
                    <img 
                        src="${product.image}" 
                ...
                return `<div class="pr-modal-image-emoji">${product.image}</div>`;
            } else {
                return '<div class="pr-modal-image-emoji">🛒</div>';
            }
        },
